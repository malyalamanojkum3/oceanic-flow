import {
  ...
  import { insertDescriptionOfGoodsSchema as schema } from "./schemas.zod";
  import { descriptionOfGoods as table} from "../../../db/schema/psd/descriptions-of-goods";
 ...
 const Router = createTRPCRouter({
        create: managerProcedure.input(schema).mutation(async ({ ctx, input }) => {
            try {
                const id = generateItemId(input.qualityDescription);  
        ...
        .query(async ({ ctx, input }) => {
            try {
                ...
                .where(and(
                    ...
                    ilike(table.qualityDescription, `%${input.query}%`)
                ));
            ...
            .mutation(async ({ ctx, input }) => {
                try {
                    const result = await ctx.db
                        .select()
                        .from(table)
                        .where(eq(table.qualityDescription, input));
